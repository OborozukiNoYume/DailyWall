...
### [2026-04-25 10:30] 并发回归测试初版误用只读临时数据库
- **现象**：我为 SQLite `NullPool` 修复新增并发回归测试时，第一次运行测试报 `attempt to write a readonly database`，导致验证流程中断并需要返工
- **原因**：我直接对一个新建的只读临时数据库运行测试，但没有先用可写连接把数据库切到 `WAL`，而当前连接初始化逻辑会执行 `PRAGMA journal_mode=WAL`，因此在只读连接阶段触发错误
- **解决**：先用可写 `sqlite3` 连接初始化临时数据库并设置 `PRAGMA journal_mode=WAL`，再创建只读引擎运行并发测试
- **教训**：为 SQLite 只读连接写测试时，不能只模拟 `mode=ro`，还要同步构造与项目运行时一致的数据库前置状态，尤其是 `WAL` 模式
